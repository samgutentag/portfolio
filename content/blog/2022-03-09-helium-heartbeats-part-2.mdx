...
summary: "Utilizing the Helium API with Python `requests`"
...
Clone the repo to follow along and run the script yourself.
...
<TerminalBlock language="bash" title="create virtual environment">
...
<TerminalBlock language="bash" title="postactivate">
{`#!/bin/zsh
...
<TerminalBlock language="bash" title="predeactivate">
#!/bin/zsh
...
<TerminalBlock language="bash" title="activate virtual environment">
workon helium-heartbeat
</TerminalBlock>

<Callout variant="tip">Quick Check! Run `printenv | grep WALLET` to ensure your environment variables are set correctly! Your Wallet Address will print to the console if everything is ready to go.</Callout>
...
<TerminalBlock language="bash" title="install requirements">
...
The `helium-heartbeats.py` script uses a multithreaded approach to get the Hotspot Heartbeat of each
...
Next, the script saves time by dividing the work to check the Heartbeat of each Hotspot across
...
The `record_heartbeat_data()` function is used to record the heartbeat data to a json file on the
local machine.
