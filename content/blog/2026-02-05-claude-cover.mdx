...
<Callout variant="tip">**Start with the simplest integration.** Open-Meteo let me focus on building rather than fighting with auth and API
keys. When you're learning, reducing friction at every step matters.</Callout>
...
<Callout variant="gotcha">**Two-step grid lookup.** NWS doesn't accept lat/lon for weather
data. You first convert coordinates to a grid point, then query that grid:</Callout>
...
<Callout variant="gotcha">**Coordinate precision causes redirects.** Send too many decimal
...
figured out I needed both `round(LATITUDE, 4)` and `follow_redirects=True`.</Callout>

<Callout variant="gotcha">**"Current" data isn't current.** The skyCover values come as
...
entry in the list - which was always the oldest, stale data.</Callout>
...
<Callout variant="tip">**Test against real APIs early.** Every one of these NWS gotchas would have been invisible in mocked tests. I
found them because I hit the real API during development. For a learning
project, integration tests that use real endpoints catch real problems.</Callout>
...
<Callout variant="tip">**Build visual debug tools early.** For any CV project, the ability
to see what the algorithm is "seeing" isn't a nice-to-have. It's essential.</Callout>
...
<Callout variant="tip">**Simple CV can be surprisingly accurate.** You don't need machine
learning to get useful results. A brightness threshold gets you remarkably far
for a "is it cloudy?" question.</Callout>
...
_Built with Python, FastAPI, and Claude Code._

<GitHubCard repo="samgutentag/cloud-cover" description="Real-time cloud cover estimation using satellite imagery and weather APIs" language="Python" />
