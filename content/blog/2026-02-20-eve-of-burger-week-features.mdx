---
title: "👨🏻‍💻 Eve-of-Event Feature Rush"
...
- **Dice emoji** — looked out of place next to the printer's line-art SVG
- **Text pill "Pick for me"** — too wide, broke the visual balance
- **"RANDOM" in a circle** — tiny uppercase text in a 44px circle, hard to read
- **Compass spin SVG** — nice but too generic
- **Magic wand SVG** — fun but unclear what it does
- **Burger emoji** — perfect. It's the theme emoji from `config.js`, so it's fork-friendly too

Six iterations on a button icon in maybe five minutes. Each one was a single exchange: "try this instead" → instant change → reload → "hmm, how about..." The feedback loop with Claude Code is fast enough that aesthetic decisions don't feel expensive.
...
The map is live at [sbburgerweekmap.com](https://sbburgerweekmap.com). Stats at [sbburgerweekmap.com/stats](https://sbburgerweekmap.com/stats).
