---
title: "🏗️ Shipping Before the Data's Ready"
...
Santa Burger Week starts February 19th. The [Independent article](https://www.independent.com/2026/02/05/burger-week-2026/) dropped on the 5th with restaurant names and addresses, but most burger descriptions were still trickling in. Some restaurants had full menu details. Some had a name but no description. Some had nothing at all.

In [another blog post](./2026-02-10-let-claude-cook-burgers) i wrote about making an interactive single page map, and I wanted the site live _now_ so people could start browsing restaurants and planning, but I didn't want to show incomplete data as if it were final. The app needed to handle three states gracefully: full details, partial details, and no details at all.

## From One Burger to Many

The original data structure had a single `menuItem` string per restaurant that worked just fine when every place had one burger. Then I looked at the actual data more carefully. Mesa Burger has three burgers. Arnoldi's has two. Several restaurants were doing multiple specials it turned out.
...
This was a bigger change than it sounds given every place that referenced the old `menuItem` string: the popup HTML, the sidebar subtitles, the search filter, the print layout, needed to iterate over an array instead. The embed has its own copy of the JS, so that's double the touch points. Claude Code handled the rename across all files in one pass, but the real work was deciding what the empty states should _look_ like in each context.

## Date-Gated Data Loading

With the data structure sorted, I had a new problem: I had two versions of the data file. `data.js` was the skeleton, restaurant names, addresses, and coordinates, but with empty `menuItems` arrays. Thus, the new file `data-2026.js` was born! I had the real menu details I was filling in as restaurants confirmed their burgers.
...
Before that date, the site loads `data.js` (skeleton). At 12:01 AM on February 19th, it switches to `data-2026.js` (full details).

<Callout variant="tip">There's also a `?year=2026` URL parameter that forces a specific data file, handy for testing the full data before launch day. If the forced file doesn't exist, it redirects back to the base URL instead of showing a broken page.</Callout>

## The About Modal

The original header had a lot crammed into it: the event name, a link to the source article, and a Venmo tip link. On mobile it was getting tight, and I wanted to add more links (the embed page, the GitHub repo, a [stats page](./2026-02-19-whos-actually-clicking)) without making the header a navigation bar.
...
Now every restaurant gets a URL anchor based on its name. Open a popup and the URL updates to `sbburgerweekmap.com/#mesa-burger-state-st`. Share that link and it flies directly to the restaurant and opens its popup.
...
The map is live at [sbburgerweekmap.com](https://sbburgerweekmap.com).

<GitHubCard repo="samgutentag/sbburgerweek" description="Santa Barbara Burger Week voting app — Claude-built, fork-ready" language="TypeScript" />
