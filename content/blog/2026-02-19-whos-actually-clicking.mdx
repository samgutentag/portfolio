...
<Callout variant="gotcha">Testing locally surfaced a fun one. `sendBeacon` with a JSON string silently failed cross-origin. No error in the console, no failed request in the network tab, just nothing. The beacon vanished into the void.
...
Neither of these errors are visible. The beacon fires, returns `true`, and the request just doesn't arrive. It took `curl` (which worked fine) and `fetch` (which showed the CORS error) to triangulate what `sendBeacon` was silently swallowing.</Callout>

<TIL>`sendBeacon` silently fails cross-origin if you set the wrong content type. No console error, no network tab trace. Use `fetch` with `keepalive: true` instead.</TIL>

## Querying the Data

Analytics Engine exposes a SQL API. After the event, I can pull everything with:

<TerminalBlock language="bash" title="query analytics engine">
