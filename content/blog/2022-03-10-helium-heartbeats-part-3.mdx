...
summary: "Pythons, Pandas and Matplots, Oh My!"
publishedAt: "2022-03-10"
---

## Visualizing The Heartbeat Data

The final script `helium_heartbeat_plots.py` is used to visualize the data the `helium_heartbeat.py` script is collecting.

<GitHubCard repo="samgutentag/helium-heartbeat" description="Monitor Helium hotspot activity and get push notifications on status changes" language="Python" />
...
The first step to visualizing the data is loading the data! The `load_data()` function does exactly
that.

The function will _find_ all of the Heartbeat JSON files in the `./data` directory, but will only
plot the last specified `n` days. For efficiency, the script assumes that a day includes at max 144
...
<Callout variant="note">Make note of how frequently you are checking for Heartbeats and adjust the values here as needed, a 10 minute interval is plenty of information.</Callout>
...
higher, the Hotspot has been inactive longer. These two Hotspots are in good shape. The line is
blue, and below the `dashed line` at the "inactive threshold" (in this case 450, which is roughly
...
In this example, the second Hotspot has crossed the threshold and is now considered Inactive.
...
| `days_back`         | how many days of data to include, default is 3                  |
| `warning_threshold` | inactive block heights over this value are considered inactive |
```

<Callout variant="note">This function is long, please see the file in the Github repo for all of the details, the snippet code here is truncated for clarity.</Callout>
...
def plot_data(data=None, days_back=3, warning_threshold=450):
    ...
    for idx, hotspot in enumerate(sorted(data.name.unique())):
        ...
        # get latest inactive count to determine line color and fill
        ...
        # if inactive is greater than warning threshold, fill in the area under the line
...
- [Monitor Helium Hotspot Activity Part 1, Overview][helium-heartbeat-part-1]
- [Monitor Helium Hotspot Activity Part 2, Using the Helium API][helium-heartbeat-part-2]
- [Monitor Helium Hotspot Activity Part 3, Visualize the Data][helium-heartbeat-part-3] (you are
  here)
- [Monitor Helium Hotspot Activity Part 4, Notifications with Pushover][helium-heartbeat-part-4]

[github-repo]: https://github.com/samgutentag/helium-heartbeat
[helium-heartbeat-part-1]: https://gutentag.co/3MzZNAb
[helium-heartbeat-part-2]: https://gutentag.co/3MGjUwo
[helium-heartbeat-part-3]: https://gutentag.co/37DqFPL
[helium-heartbeat-part-4]: https://www.samgutentag.com/blog
