---
title: "🎨 Taming Logo SVGs at Scale"
...
<TerminalBlock language="bash" title="generate color variants">
...
gusto:       viewBox="-131 160.7 599.9 228.2"    ← was -153 -46 652 652
waabi:       viewBox="0.4 43 1753.42 499.88"      ← was 0 0 1755 586
glydways:    viewBox="11.83 11.99 309.14 35.94"   ← was 0 0 333.17 56.806
```
...
<Callout variant="gotcha">The script works great for SVGs where the path coordinates match the viewBox coordinate space. It doesn't work when SVG elements are inside a `<g>` with a `transform` attribute.
...
Cockroach Labs uses `fill="url(#pattern)"` references with pattern transforms. The script saw paths inside `<defs>` that define pattern tiles and included their coordinates in the bbox, making it larger than the original.</Callout>
...
**Ellipse group** (the dot grid icon) has `transform="translate(7.283 1.558)scale(.1176)"`. The rightmost ellipse is at `cx=236.606` with `rx=19.394`, so its right edge in local space is `256.0`. After the scale and translate: `7.283 + (256.0 × 0.1176) = 37.39`.
...
So the combined content ends at x=149.07, but the viewBox was `0 0 178 42` — 29 pixels of nothing. Updated to `viewBox="0 0 150 42"` across all four variants and the logo immediately rendered larger in the scrolling bar at the same `height: 28px`.
...
<TIL>Regex-based SVG processing works for a one-time job but is fundamentally fragile. For production use, parse SVGs as XML and walk the DOM. Use `resvg` for accurate bounding boxes.</TIL>
